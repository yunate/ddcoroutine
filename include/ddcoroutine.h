...
public:
    ...
    auto operator co_await() const& noexcept
    {
        struct ddawaiter
        {
            ...
            {
                assert(m_self_ctx != nullptr);
                m_self_ctx->caller_co_handle = caller_handle.address();
                if (m_self_ctx->check_unset_and_set(detail::ddflag_set_point::set_in_co_await)) {
                    // path 1(异步)final_suspend 还没有运行
                    // !!! 非常危险 !!!, 请注意!!! 考虑在此加入::Sleep(1000);
...
inline void ddcoroutine_run(const ddcoroutine<void>& r)
{
    ...
    public:
        ...
        inline void remove_coroutine(const std::coroutine_handle<>& r)
        {
            std::lock_guard<std::mutex> guard(m_mutex);
            if (m_holded_handle.find(r) != m_holded_handle.end()) {
                // `ddcoroutine_noop::final_suspend` return `std::suspend_never`, so it wll auto destroy.
...
inline void ddcoroutine_deatch(const ddcoroutine<void>& r)
{
    ddcoroutine_run(r);
}
...
// if deatch_all is true, the other ddcoroutine will be detached when the first ddcoroutine is resumed.
// if deatch_all is false, the other ddcoroutine still can be co_await.
// if you do not care about the other ddcoroutine when the first ddcoroutine is resumed, set deatch_all to true.
inline ddcoroutine<int> ddcoroutine_any(std::vector<ddcoroutine<void>> r, bool deatch_all = true)
{
    if (r.empty()) {
        co_return -1;
    }
        ...
        void resume(int index)
        {
            ...
            for (auto& it : coroutines) {
                ...
                if (awaiter.m_self_ctx->set_point_flag.compare_exchange_strong(expect, (unsigned char)detail::ddflag_set_point::not_set, std::memory_order_acq_rel)) {
                    ...
                    awaiter.m_self_ctx->caller_co_handle = nullptr;
                    delete resumer;
                }
            }
    ...
    struct ddawaiter
    {
        ...
        bool await_suspend(std::coroutine_handle<> caller_handle) noexcept
        {
            ...
            for (size_t i = 0; i < any_ctx->coroutines.size(); ++i) {
                ...
                awaiter.m_self_ctx->caller_co_handle = resumer;
